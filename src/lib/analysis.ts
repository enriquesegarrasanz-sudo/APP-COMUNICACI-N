...
export async function getAiCoachNotes(transcript: string) {
  const apiKey = process.env.OPENAI_API_KEY;

  if (!apiKey || transcript.trim().length < 40) {
    return null;
  }
      ...
      messages: [
        {
          role: "system",
          content:
            "Eres coach de oratoria. Devuelve solo JSON con una propiedad notes que sea un array de 3 consejos breves, concretos y accionables en espanol.",
        },
        {
          role: "user",
  ...
  return Array.isArray(parsed.notes) ? parsed.notes.slice(0, 3) : null;
}
